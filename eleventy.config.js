...
async function imageShortcode(src, alt, sizes = '', caption = '', width = '') {
    ...
    // Validation checks.
    if (alt === undefined) throw new Error(`Missing \`alt\` on image: ${fullSrc}`)
...
async function galleryWithTextShortcode(content, ...args) {
    // Separate out the description (last argument)
    const text = content.trim();
    const images = args;

    const pageDir = './web/posts/terminology'; // Adjust to match your Markdown directory
    const outputDir = 'build/terminology';     // Adjust if needed
    const urlPath = '/terminology';            // Adjust if needed

    const html = await Promise.all(images.map(async (file, index, arr) => {
        ...
        const imageAttributes = {
            ...
            loading: "lazy",
            decoding: "async",
            pagefindSrc: `${urlPath}/${fileName}-200w.webp`,
            pageFindAlt: fileName,
            "data-pagefind-meta": `image[pagefindSrc]`,
        };
        ...
        const imageHtmlWithCaption = wrapImageInFigure(
            imageHtml,
            '', // No caption in this layout
            'width: 100%;'
        );

        return `<div class="slide fade" style="display: flex; width: 100%;">
            <span class="carousel__index rounded">${index + 1} / ${arr.length}</span>
...
return `
<div class="flex mobile-column" style="gap: 2rem;" data-gallery="${galleryId}">
    <div class="image-container flex rounded mobile-30">
        ${imageBlock}
    </div>
...
async function galleryShortcode(speciesName, caption = '') {
    ...
    const html = await Promise.all(imageFiles.map(async function (file, index, arr) {
        const fileName = file.replace(/\.[^/.]+$/, "");
        ...
        const imageHtmlWithCaption = wrapImageInFigure(
            imageHtml,
            imageTitle ? `${imageTitle}<br>${imageAttribution}` : imageAttribution,
            'width: 100%;'
        );
...
async function speciesImageShortCode(speciesName, alt, sizes = '', caption = '', width = '') {
    ...
    if (fs.existsSync(jpgPath)) {
        ...
        console.warn(`Image not found: ${jpgPath} or ${pngPath}, using placeholder.`);
        const html = `<img src="${placeholderSrc}" alt="${alt}" style="width:${width};" loading="lazy" decoding="async" class="fit-image">`;
        return wrapImageInFigure(html, caption);
    }
...
async function speciesThumbnailShortCode(speciesName, alt, sizes = '', caption = '', width = '') {
    ...
    try {
        ...
        console.warn(`Image not found for species "${speciesName}", using placeholder.`);
...
async function genusImageShortCode(genusName, alt, sizes = '', caption = '', width = '') {
...
async function familiaImageShortCode(familiaName, alt, sizes = '', caption = '', width = '') {
...
async function absoluteImageShortcode(src, name, alt, sizes = '', caption = '', width = '') {
...
module.exports = eleventyConfig => {
    ...
    eleventyConfig.addAsyncShortcode("speciesThumbnail",speciesThumbnailShortCode);
    ...
    eleventyConfig.addPairedAsyncShortcode("galleryWithText", galleryWithTextShortcode);

    // Collections.
    eleventyConfig.addCollection("posts", (collection) => {
        return collection.getFilteredByGlob("./web/posts/**/*.md");
    });
    ...
    eleventyConfig.addWatchTarget('./web/posts');
...
outputFiles.forEach((file) => {
    ...
    eleventyConfig.addPassthroughCopy("README.md")
        ...
        eleventyConfig.addFilter("checkFamiliaImage", function(name) {
          ...
          const imagePath = path.join(__dirname, "data/familiae/", `${name}/`, `${name}.jpg`);
        ...
        eleventyConfig.addFilter("checkGenusImage", function(name) {
            ...
            // Path where images are stored
            const imagePath = path.join(__dirname, "data/genera/", `${name}/`, `${name}.jpg`);
          ...
          eleventyConfig.addFilter("inlineGeoJSON", function (regionsObj, speciesSlug) {
            ...
            for (const level of [1, 2, 3, 4]) {
              const levelPath = path.join(__dirname, `data/geojson/level${level}.geojson`);
